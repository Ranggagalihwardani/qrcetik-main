...
@section('content')
  <div class="overflow-hidden rounded-2xl bg-white/10 border border-white/20 backdrop-blur-xl shadow-lg p-6">
    <h2 class="text-xl font-bold mb-6">Buat Dokumen</h2>

    <form id="docForm" method="post" action="{{ route('documents.store') }}" class="space-y-6">
      @csrf
      ...
      <div>
        <div class="flex items-center justify-between mb-2">
          <label class="block text-sm font-semibold">Isi Dokumen (bebas)</label>
          <button type="button" id="btnInsertQr" class="text-xs underline opacity-80 hover:opacity-100">
            Sisipkan placeholder QR
          </button>
        </div>
        <textarea id="editor" name="html_template">{{ old('html_template', $default ?? '') }}</textarea>
...
tanggal=2025-09-30">{{ old('payload') }}</textarea>
  ...
  <script>
    ...
    // Penting: sinkronisasi konten editor saat submit
